...
enum ScreenType: Equatable {
    ...
    case largeProfile(btnName: NaviButtonType = .alert, btnName2: NaviButtonType = .gear)
    ...
    var buttonImage: String {
        ...
        case .largeProfile(let btnName, _),
    ...
    var buttonImage2: String {
        switch self {
        case .largeProfile(_, let btnName2):
            return btnName2.buttonName
        default:
            return ""
        }
    }
...
class CustomNavigationView: UIView {
    ...
    private lazy var rightFirstButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(smallFirstRightBtnDidTap), for: .touchUpInside)
        button.translatesAutoresizingMaskIntoConstraints = false
        return button
    }()
    ...
    private lazy var rightSecondButton: UIButton = {
        let button = UIButton(frame: CGRect(x: 0, y: 0, width: 36, height: 36))
        // 변경된 부분: buttonImage2 사용
        if let image = UIImage(systemName: self.screenType.buttonImage2) {
            button.setImage(image, for: .normal)
        } else {
            button.setTitle(self.screenType.buttonImage2, for: .normal)
        }
        button.tintColor = .black
        button.addTarget(self, action: #selector(smallSecondRightBtnDidTap), for: .touchUpInside)
        button.translatesAutoresizingMaskIntoConstraints = false
        return button
    }()
    ...
    private func naviLargeSizeSetup() {
        ...
        // 기존의 서브뷰 제거 후 조건에 따라 버튼 추가
        rightButtonSV.arrangedSubviews.forEach { $0.removeFromSuperview() }
        
        // 첫 번째 버튼 추가
        rightButtonSV.addArrangedSubview(rightFirstButton)
        
        // largeProfile 화면에서만 두 번째 버튼 추가
        if screenType == .largeProfile() {
            rightButtonSV.addArrangedSubview(rightSecondButton)
        }
    ...
    private func naviSmallSizeSetup() {
        ...
        // largeProfile 화면에서는 두 번째 버튼을 추가하지 않음
        // small 사이즈에서는 두 번째 버튼을 사용하지 않음
    ...
    @objc func smallSecondRightBtnDidTap() {
        self.delegate?.smallSecondRightBtnDidTap()
    }
    
    @objc func smallFirstRightBtnDidTap() {
        self.delegate?.smallFirstRightBtnDidTap()
    }
